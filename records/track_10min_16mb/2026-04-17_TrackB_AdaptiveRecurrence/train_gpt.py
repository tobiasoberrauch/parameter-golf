...
class Hyperparameters():
    ...
    # Adaptive SDClip
    adaptive_sdclip = bool(int(os.environ.get('ADAPTIVE_SDCLIP', '1')))
    sdclip_lambda = float(os.environ.get('SDCLIP_LAMBDA', 0.175))
    ...
    mod_capacity = float(os.environ.get('MOD_CAPACITY', 0.75))  # fraction of tokens processed
    mod_start_layer = int(os.environ.get('MOD_START_LAYER', 4))
    mod_end_layer = int(os.environ.get('MOD_END_LAYER', 8))
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor) -> Tensor:
        ...
        if self.parallel:
            # GPT-J style: attention and MLP both read from same input
            normed = self.attn_norm(x_in) * self.ln_scale_factor
            attn_out = self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * self.attn(normed)
...
class DepthRouter(nn.Module):
    """Lightweight binary router for mixture-of-depths."""
    def __init__(self, dim: int, capacity: float = 0.75):
        super().__init__()
        self.gate = nn.Linear(dim, 1, bias=False)
        self.capacity = capacity
        nn.init.zeros_(self.gate.weight)  # Start with no routing (all tokens processed)

    def forward(self, x: Tensor) -> tuple[Tensor, Tensor, Tensor]:
        """Returns (selected_x, indices, full_shape) for scatter-back."""
        B, T, D = x.shape
        scores = self.gate(x).squeeze(-1)  # (B, T)
        k = max(1, int(T * self.capacity))
        _, indices = scores.topk(k, dim=-1, sorted=False)  # (B, k)
        indices_sorted, _ = indices.sort(dim=-1)  # maintain causal order
        # Gather selected tokens
        selected = x.gather(1, indices_sorted.unsqueeze(-1).expand(-1, -1, D))
        return selected, indices_sorted, (B, T, D)

    @staticmethod
    def scatter_back(x_full: Tensor, x_processed: Tensor, indices: Tensor) -> Tensor:
        """Scatter processed tokens back into full sequence."""
        return x_full.scatter(1, indices.unsqueeze(-1).expand_as(x_processed), x_processed)
...
class GPT(nn.Module):
    def __init__(self, h: Hyperparameters):
        ...
        # Mixture-of-Depths routers
        self.routers = nn.ModuleDict()
        if h.mod_enabled:
            for i in range(h.mod_start_layer, min(h.mod_end_layer + 1, h.num_layers)):
                self.routers[str(i)] = DepthRouter(h.model_dim, h.mod_capacity)
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        for i in enc_iter:
            router = self.routers.get(str(i)) if hasattr(self, 'routers') else None
            if router is not None and self.training:
                x_selected, indices, full_shape = router(x)
                # Process only selected tokens
                x0_selected = x0.gather(1, indices.unsqueeze(-1).expand(-1, -1, x0.shape[-1]))
                out_selected = self.blocks[i](x_selected, x0_selected)
                x = DepthRouter.scatter_back(x, out_selected, indices)
            else:
...
def compute_adaptive_clip_sigmas(hessians: dict, base_k: float, lambda_adapt: float) -> dict:
    """Modulate SDClip k per layer group based on Hessian trace.

    Layers with higher Hessian trace (more sensitive) get higher k (wider clip = lower entropy).
    k_i = base_k * (1 + lambda * (trace_i / mean_trace - 1))
    """
    # Group traces by layer
    layer_traces = {}
    for name, H in hessians.items():
        # Extract layer index from name like 'blocks.3.attn.c_q.weight'
        parts = name.split('.')
        for i, p in enumerate(parts):
            if p == 'blocks' and i + 1 < len(parts) and parts[i+1].isdigit():
                layer_idx = int(parts[i+1])
                layer_traces[layer_idx] = layer_traces.get(layer_idx, 0.0) + float(H.trace().item())
                break

    if not layer_traces:
        return {}

    mean_trace = sum(layer_traces.values()) / len(layer_traces)
    adaptive_k = {}
    for layer_idx, trace in layer_traces.items():
        adaptive_k[layer_idx] = base_k * (1.0 + lambda_adapt * (trace / max(mean_trace, 1e-10) - 1.0))

    return adaptive_k


def get_clip_sigmas_for_param(name: str, adaptive_k: dict, default_k: float) -> float:
    """Look up the adaptive clip sigma for a parameter based on its layer index."""
    parts = name.split('.')
    for i, p in enumerate(parts):
        if p == 'blocks' and i + 1 < len(parts) and parts[i+1].isdigit():
            layer_idx = int(parts[i+1])
            return adaptive_k.get(layer_idx, default_k)
    return default_k
...
def gptq_mixed_quantize(
    ...
    # Compute per-layer-group adaptive clip sigmas from Hessian traces
    adaptive_k = {}
    if h.adaptive_sdclip:
        adaptive_k = compute_adaptive_clip_sigmas(hessians, h.matrix_clip_sigmas, h.sdclip_lambda)
        if adaptive_k:
            log(f"Adaptive SDClip: modulating k across {len(adaptive_k)} layer groups "
                f"(lambda={h.sdclip_lambda}, base_k={h.matrix_clip_sigmas})")
            k_values = sorted(adaptive_k.values())
            log(f"  k range: [{k_values[0]:.3f}, {k_values[-1]:.3f}]")
    ...
    for name, tensor in state_dict.items():
        ...
        if "tok_emb" in name:
            ...
            cs = get_clip_sigmas_for_param(name, adaptive_k, h.matrix_clip_sigmas)
        else:
