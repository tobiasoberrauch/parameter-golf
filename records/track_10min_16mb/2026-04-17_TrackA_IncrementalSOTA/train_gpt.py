...
def eval_with_ttt(
    ...
    """Score-first TTT: score entire chunk, then SGD-adapt, repeat for next chunk.

    Legal compliance:
    - Strictly causal sliding-window eval
    - Standard softmax (no logit biasing)
    - Score before update (tokens scored under no_grad before adaptation)
    - Single-pass scoring (no rescoring)
    """
    import copy as _copy
    # Work on a deep copy so the original model is untouched after TTT
    model = _copy.deepcopy(base_model)
    model.eval()

    val_tokens = val_data.val_tokens  # full validation tensor
    seq_len = h.eval_seq_len
    ...
    for chunk_start in range(0, val_tokens.numel() - 1, chunk_size):
        chunk_end = min(chunk_start + chunk_size, val_tokens.numel() - 1)
        chunk = val_tokens[chunk_start:chunk_end + 1]

        # PHASE 1: Score all windows in this chunk (no gradients)
        model.eval()
        with torch.no_grad():
            num_seqs = (chunk.numel() - 1) // seq_len
            if num_seqs == 0:
                continue
            local_batch_seqs = max(1, h.val_batch_tokens // (seq_len * h.world_size))
            for seq_idx in range(0, num_seqs, local_batch_seqs):
                batch_end = min(seq_idx + local_batch_seqs, num_seqs)
                raw_start = seq_idx * seq_len
                raw_end = batch_end * seq_len + 1
                if raw_end > chunk.numel():
                    break
                batch = chunk[raw_start:raw_end].to(device=device, dtype=torch.int64, non_blocking=True)
                x = batch[:-1].reshape(-1, seq_len)
                y = batch[1:].reshape(-1, seq_len)
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                ...
                # BPB calculation (using GPU LUTs like eval_val_sliding)
                prev_ids = x.reshape(-1)
                tgt_ids = y.reshape(-1)
                token_bytes = val_data.base_bytes_lut[tgt_ids].to(torch.float64)
                token_bytes += (val_data.has_leading_space_lut[tgt_ids] &
                                ~val_data.is_boundary_token_lut[prev_ids]).to(torch.float64)
                total_bytes += token_bytes.sum().item()

        # PHASE 2: SGD adaptation on this chunk (after scoring)
        if h.ttt_enabled and h.ttt_epochs > 0:
            model.train()
            # Differentiated LR per parameter group
            embed_params = []
            matrix_params = []
            scalar_params = []
            for name, param in model.named_parameters():
            ...
            optimizer = torch.optim.SGD([
                {'params': embed_params, 'lr': h.ttt_lr * 0.1},      # embeddings: conservative
                {'params': matrix_params, 'lr': h.ttt_lr * 1.5},     # attention/MLP: aggressive
                {'params': scalar_params, 'lr': h.ttt_lr * 0.5},     # scales/gains: moderate
            ], momentum=h.ttt_momentum)

            for epoch in range(h.ttt_epochs):
                # Cosine LR decay — scale all groups proportionally
                decay = 0.5 * (1 + math.cos(math.pi * epoch / h.ttt_epochs))
                base_lrs = [h.ttt_lr * 0.1, h.ttt_lr * 1.5, h.ttt_lr * 0.5]
                ...
                for seq_idx in range(num_seqs):
                    ...
                    with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                        loss = model(x, y)
                    loss.backward()
                    torch.nn.utils.clip_grad_norm_(model.parameters(), h.ttt_grad_clip)
                    optimizer.step()
                    optimizer.zero_grad()

        if log_fn and chunk_start % (chunk_size * 10) == 0:
            log_fn(f"ttt_progress: chunk {chunk_start // chunk_size}/{(val_tokens.numel() - 1) // chunk_size}")
...
def train_and_eval(h: Hyperparameters, device: torch.device) -> None:
    ...
    if h.ttt_enabled and h.is_main_process:
        log("Starting TTT evaluation...")
        ttt_t0 = time.perf_counter()
        ttt_val_loss, ttt_val_bpb = eval_with_ttt(h, device, val_data, eval_model, log_fn=log)
        torch.cuda.synchronize()
        ttt_ms = 1000.0 * (time.perf_counter() - ttt_t0)
        log(f"final_ttt val_loss:{ttt_val_loss:.4f} val_bpb:{ttt_val_bpb:.4f} eval_time:{ttt_ms:.0f}ms")
