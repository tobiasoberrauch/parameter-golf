...
class Hyperparameters:
    ...
    qk_gain_init: float = float(os.environ.get("QK_GAIN_INIT", 5.5))
    parallel_residual_start: int = int(os.environ.get("PARALLEL_RESIDUAL_START", 7))
...
class Block(nn.Module):
    ...
    def __call__(self, x: mx.array, x0: mx.array) -> mx.array:
        ...
        if self.parallel:
            ...
            attn_out = self.attn_scale.astype(x_in.dtype)[None, None, :] * self.attn(normed)
            mlp_out = self.mlp_scale.astype(x_in.dtype)[None, None, :] * self.mlp(self.mlp_norm(x_in))
            return x_in + attn_out + mlp_out
        else:
            attn_out = self.attn(self.attn_norm(x_in))
            x = x_in + self.attn_scale.astype(x_in.dtype)[None, None, :] * attn_out
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 ...
                 qk_gain_init: float, parallel_residual_start: int = 7,
                 num_loops: int = 0, loop_start: int = 3, loop_end: int = 5):
        super().__init__()
        ...
        if num_loops > 0:
            ...
            self.loop_encoder_indices = all_indices[:num_enc]
            self.loop_decoder_indices = all_indices[num_enc:]
        else:
            self.loop_encoder_indices = list(range(self.num_encoder_layers))
            self.loop_decoder_indices = list(range(self.num_encoder_layers, num_layers))
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        ...
        enc_iter = self.loop_encoder_indices if self.looping_active else range(self.num_encoder_layers)
        dec_iter = self.loop_decoder_indices if self.looping_active else range(self.num_encoder_layers, self.num_encoder_layers + self.num_decoder_layers)
...
def main() -> None:
    ...
    while True:
        ...
        # Activate layer looping once training fraction passes the threshold.
        frac = step / args.iterations
        if args.num_loops > 0 and not model.looping_active and frac >= args.enable_looping_at:
            model.looping_active = True
            log(f"Activating layer looping at step {step} (frac={frac:.3f})")
